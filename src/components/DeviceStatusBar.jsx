  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 14, fontWeight: 700, color: '#f5f2ee', lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {profile?.username ?? 'Joueur'}
        </p>
        {profile?.location && (
          <p style={{ fontSize: 12, color: 'rgba(245,242,238,0.5)' }}>📍 {profile.location}</p>
        )}
      </div>
      ...
      {characters.length > 1 && (
        <button onClick={switchCharacter} title="Changer de personnage" style={{
          ...
          borderRadius: 11, padding: '8px 16px', fontSize: 13, fontWeight: 700,
